...
SimpleCmd* handleSimpleCmdStr(int begin, int end){
    ...
    char c, buff[10][40], inputFile[30], outputFile[30], progBuff[30], *temp = NULL;
    SimpleCmd *cmd = (SimpleCmd*)malloc(sizeof(SimpleCmd));

	//默认为非后台命令，输入输出重定向为null
    cmd->isBack = 0;
    cmd->input = cmd->output = NULL;//cmd->progB =
    ...
    //如果有管道，则为变量赋值
    if(strlen(progBuff) != 0){
	cmd->progB = (char**)malloc(sizeof(char*)*2);
	cmd->progB[1] = NULL;
        j = strlen(progBuff);
        cmd->progB[0] = (char*)malloc(sizeof(char) * (j + 1));
        strcpy(cmd->progB[0], progBuff);
    }
...
void execOuterCmd(SimpleCmd *cmd){
	...
	if(cmd->progB == NULL){
		...
		;
	    char **prog1_argv = (char**)malloc(sizeof(char*)*(len+1));
	    for(i = 0;i < len;i++){
		j = strlen(cmd->args[i]);
		prog1_argv[i] = (char*)malloc(sizeof(char)*(j+1));
		strcpy(prog1_argv[i],cmd->args[i]);
	    }

	    prog2_argv[0] = cmd->progB[0];
	    prog2_argv[1] = cmd->progB[1];
...
void execSimpleCmd(SimpleCmd *cmd){
	...
	int flag = 0;
	temp = cmd->args[1];
	if(strcmp(temp,"exit")==0 || strcmp(temp,"history")==0 ||
	  strcmp(temp,"jobs")==0 || strcmp(temp,"cd")==0 ||
	  strcmp(temp,"fg")==0 || strcmp(temp,"bg")==0 ||
	  strcmp(temp,"echo")==0 || strcmp(temp,"type")==0){
	    flag = 1;
